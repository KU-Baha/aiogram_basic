...
#     answer = (f'{call.message.from_user.full_name}, ты выбрал MacBook {model} с диагональю экрана'
#               f' {size} дюймов, процессором {chip} и годом выпуска {year}')
...
async def select_macbook(call: CallbackQuery, bot: Bot, callback_data: MacInfo):
    ...
    answer = (f'Ты выбрал MacBook {model} с диагональю экрана'
              f' {size} дюймов, процессором {chip} и годом выпуска {year}')
